...
in  { twoNamespaces.basicTest
      =
        let result = separate dataNamespace [ listNamespace ]

        in  assert : toText result === "Data.List"
    , threeNamespaces.basicTest
      =
        let result =
              separate controlNamespace [ monadNamespace, readerNamespace ]

        in  assert : toText result === "Control.Monad.Reader"
    , singleNamespaceWithEmpty.basicTest
      =
        let result = separate dataNamespace ([] : List NamespaceType)
    ...
    , multipleNamespaces.fourNamespaces
      =
        ...
        in  assert : toText result === "Data.List.Utils.Helpers"
    }
